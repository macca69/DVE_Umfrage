...
def plot_and_layout(fig_data, filter1, filter2, barnorm):
    ...
    fig.update_layout(legend=dict(itemwidth=30, title_text='', font_size=15),
                     ...
                     #legend_font_size=15,
                     font=dict(size=15))
    fig.update_xaxes(title=filter1.split(') ')[1], titlefont_size=20, tickfont_size=15, categoryarray=natsorted(fig_data[filter1].unique()), categoryorder='array')
...
def significance_test(df, filter1, filter2, filter1_items, filter2_items):
    ...
    with st.expander('Einzelheiten Signifikanztest'):
        st.write(filter1, filter2)
        st.write(filter1_items, [x for x in filter1_items if x == x])
        st.write(filter2_items, [x for x in filter2_items if x == x])
        filter1_items = [x for x in filter1_items if x == x]
        filter2_items = [x for x in filter2_items if x == x]
