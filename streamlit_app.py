...
for ii in filter1_items:
    temporary_1 = df_1[df_1[filter1]==ii].apply(string_to_list)
    if filter2 in mehrfach:
        st.write(pd.Series(flatten([k for k in temporary_1[filter2]])).value_counts().to_frame().assign(filter1=ii))
